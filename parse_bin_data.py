"""
 * Parse raw .bin files containing raw ADC samples to .npy files.
 * Authors: Ryu Okubo, Jizheng He
"""
...
binFilePath = "/Users/djbautista/Desktop/437/radar/Industrial_Visualizer/binData/12_12_2023_19_31_07/"

binFileName = "pHistBytes"
if binFilePath[-1] == "/":
    binFilePath = binFilePath[:-1]
outputName = "data_" + os.path.basename(binFilePath) + ".npy"
number_of_files = len(os.listdir(binFilePath))
numAntennas = 3
...
def parse_ADC(binFilePath, output_file=None):
    data = np.array([], dtype=frameStatStruct) #file to be returned
    targetFrameNum = 0
    ...
    for file_num in range(1, number_of_files + 1):
        ...
        # open file and read all the ADC data
        with open(thisBinFileName, 'rb') as fid:
            ...
            while True:
                ...
                data = np.append(data, fHist)

    if output_file:
        print("Saving data of {} frames to {}".format(data.shape[0], output_file))
        np.save(output_file, data, allow_pickle=True)
    return data
    

if __name__ == "__main__":
    parse_ADC(binFilePath, outputName)
